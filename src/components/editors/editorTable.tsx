...
export function EditorTable({ editors, onEdit, onDelete }: EditorTableProps) {
  const getRoleBadgeVariant = (role: Editor["role"]) => {
    switch (role) {
      case "Editor":
        return "outline"
      default:
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>A list of all editors in your team including their name, email and role.</CardDescription>
      </CardHeader>
      <CardContent>
        {editors.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Get started by adding your first editor.</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {editors.map((editor) => (
                <TableRow key={editor.id}>
                  ...
                  <TableCell>
                    <Badge variant={"default"}>{editor.role}</Badge>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="sm" onClick={() => onEdit(editor)} className="h-8 w-8 p-0">
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              <span className="font-medium">{editor.name}</span> from your team.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
